    ...
    <div className={`flex gap-3 animate-fade-in ${isUser ? "flex-row-reverse" : ""}`}>
      ...
      >
        ...
        {isUser ? (
          ...
          // Detect the professional-advice disclaimer (added by the AI only for
          // engineering/legal/safety assessments) and render it as a neutral card.
          const disclaimerRe = /\n?\s*Bilgi:\s*Bu değerlendirme[^\n]*?yetkili uzman tarafından verilmelidir\.?\s*$/;
          const match = message.content.match(disclaimerRe);
          const body = match ? message.content.replace(disclaimerRe, "").trimEnd() : message.content;
          ...
          return (
            <>
              ...
              {disclaimer && (
                <div className="mt-3 rounded-lg border border-border/60 bg-muted/40 px-3 py-2 text-[11px] leading-relaxed text-muted-foreground">
                  {disclaimer}
                </div>
              )}
            </>
