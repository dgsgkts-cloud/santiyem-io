...
async function chargeStoredCard(sub: any, supabaseAdmin: any): Promise<boolean> {
  ...
  const requestBody = {
    ...
    buyer: {
      ...
      gsmNumber: '+905000000000', email: sub.user_email || 'user@santiyem.io',
    ...
    basketItems: [{ id: sub.plan_name, name: `${sub.plan_name} Plan`, category1: 'Subscription', itemType: 'VIRTUAL', price }],
  }
...
Deno.serve(async (req) => {
  try {
    ...
    // 1. Send reminder emails for trials ending tomorrow
    ...
    if (trialEndingSoon && trialEndingSoon.length > 0) {
      for (const sub of trialEndingSoon) {
        // Send reminder email via transactional email
        try {
          ...
          if (authUser?.user?.email) {
            ...
            await supabaseAdmin.functions.invoke('send-transactional-email', {
              body: {
                ...
                subject: 'Deneme Süreniz Yarın Sona Eriyor',
                template_data: {
                  ...
                  projectName: `${sub.plan_name === 'pro' ? 'Profesyonel' : sub.plan_name === 'team' ? 'Ekip' : 'Kurumsal'} Plan`,
                },
              },
            })
          }
    ...
    // 2. Process expired trials — charge the card
